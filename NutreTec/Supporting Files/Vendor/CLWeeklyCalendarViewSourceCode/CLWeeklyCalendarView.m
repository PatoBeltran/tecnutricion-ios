...
#define DATE_TITLE_MARGIN_TOP 15.f
...
-(UIView *)backgroundView {
    if(!_backgroundView){
        ...
        [_backgroundView addSubview:self.dayTitleSubViewContainer];
        [_backgroundView addSubview:self.dailySubViewContainer];
        [_backgroundView addSubview:self.dailyInfoSubViewContainer];
        ...
        [_backgroundView addGestureRecognizer:recognizerRight];
        
        UISwipeGestureRecognizer *recognizerLeft;
        recognizerLeft.delegate=self;
        recognizerLeft = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeLeft:)];
        [recognizerLeft setDirection:UISwipeGestureRecognizerDirectionLeft];
        [_backgroundView addGestureRecognizer:recognizerLeft];
    }
    
    _backgroundView.backgroundColor = [UIColor colorWithPatternImage:[UIImage calendarBackgroundImage:self.bounds.size.height]];
...
-(void)initDailyViews {
    ...
    NSDate *dtWeekStart = [yesterday getWeekStartDate:self.weekStartConfig.integerValue];
    ...
    [self dailyCalendarViewDidSelect:yesterday];
}
...
-(DailyCalendarView *)dailyViewForDate: (NSDate *)date inFrame: (CGRect)frame {
    ...
    [view setImage:[self.delegate infoImageForDay:date]];
...
-(void)dailyInfoViewDidClick: (UIGestureRecognizer *)tap {
    ...
    [self redrawToDate:[NSDate dateWithTimeIntervalSinceNow:-86400]];
}
...
-(void)adjustDailyInfoLabelAndWeatherIcon: (BOOL)blnShowWeatherIcon {
    if(blnShowWeatherIcon && [self.selectedDate isDateToday]){
