...
- (void)calculateAllPoints
{
    ...
    [self drawImagesWithMaxLength:maxLength imageArray:images angleArray:angleArray];
...
- (void)drawImagesWithMaxLength:(CGFloat)maxLength imageArray:(NSArray *)imageArray angleArray:(NSArray *)angleArray
{
    int imageTag = 921;
    while (true) {
        UIView *label = [self viewWithTag:imageTag];
        if (!label) break;
    ...
    CGFloat labelLength = maxLength + maxLength/4 + 2;
    for (UIImage *image in imageArray) {
        CGFloat angle = [[angleArray objectAtIndex:section] floatValue];
        CGFloat x = _centerX + labelLength*cos(angle);
        CGFloat y = _centerY + labelLength*sin(angle);
        
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(x - 3*fontSize/2, y - 3*fontSize/2, 3*fontSize, 3*fontSize)];
        imageView.backgroundColor = [UIColor clearColor];
//        imageView.transform = CGAffineTransformMakeRotation(((float)section/[imageArray count]) *
//                                                        (2*M_PI) + M_PI_2);
